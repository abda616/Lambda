...
def get_latest_ami_id():
    ...
    data = [element for element in data if
            has_tags(element, required_tags)]  # data without desiered tags will be removed
    images = sorted(data, key=parse_date,
                    reverse=True)  # order the data based on time and reverse to find the most recent date
...
def create_launch_template_version(latest_ami_id, launch_template_id):
    ...
    describe_response = ec2_client.describe_launch_templates(
        # get info about LT so I can find LatestVersionNumber -1->
    ...
    response = ec2_client.create_launch_template_version(  #create new version of LT
        ...
        # -2->  to update it in next LT version
        LaunchTemplateData={
...
def send_message(message):
    response = session.client("sns").publish(TopicArn=os.environ['sns_topic'], Message=message)


def lambda_handler(event, context):
    latest_ami_id = get_latest_ami_id()  # get ami if there is no ami ; Out else Continue

    if not latest_ami_id:
        send_message("No AMI found")
        return {
            'statusCode': 500,
